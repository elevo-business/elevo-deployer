// ELEVO DEPLOY CENTER v2.0 — deploy.elevo.solutions
...
function json(res, s, d) { res.writeHead(s, {'Content-Type':'application/json','Access-Control-Allow-Origin':'*','Access-Control-Allow-Methods':'GET,POST,DELETE,OPTIONS','Access-Control-Allow-Headers':'Content-Type,Authorization'}); res.end(JSON.stringify(d)); }
...
function github(method, p, data) {
  const payload = data ? JSON.stringify(data) : null;
  const opts = { hostname:'api.github.com', port:443, path:p, method, headers:{'Authorization':`Bearer ${GITHUB_TOKEN}`,'Accept':'application/vnd.github+json','User-Agent':'ELEVO/2','X-GitHub-Api-Version':'2022-11-28','Content-Type':'application/json'} };
  if(payload) opts.headers['Content-Length'] = Buffer.byteLength(payload);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    // Config
    if(p==='/api/config') return json(res,200,{coolify:!!COOLIFY_TOKEN,github:!!GITHUB_TOKEN,anthropic:!!ANTHROPIC_API_KEY,ghApp:ghAppUuid||'pending'});
...
server.listen(PORT, async () => {
  console.log(`\n  ELEVO Deploy Center v2.0 | Port ${PORT}\n  Coolify: ${COOLIFY_URL} | Claude: ${ANTHROPIC_API_KEY?'✓':'✗'}\n`);
  await detectGHApp();
});
